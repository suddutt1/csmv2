	...
	private void performProcessing(HttpServletRequest request,
		...
		try {
			if (this.actionConfig != null) {
				boolean isActionForward = false;
				do {
					if(!isActionForward)
					{
							requestURI= request.getRequestURI();
					}
					isActionForward = false;
					...
					ModelAndView result = (ModelAndView) method.invoke(
							actionInstance, new Object[] { request, response });
					if (result != null) {
						switch (result.getViewType()) {
						case JSP_VIEW:
							placeModelInRequestScope(result.getModelMap(),
									request);
						...
						case FORWARD_ACTION_VIEW:
							placeModelInRequestScope(result.getModelMap(),
									request);
							String forwardActionPath = (String) result
									.getView();
							requestURI = forwardActionPath;
							isActionForward = true;
							break;
						case NO_VIEW:
